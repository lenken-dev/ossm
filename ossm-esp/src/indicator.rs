...
//! of RMT and uses TX channel 0; it cannot share that peripheral with the current
...
pub async fn build(config: Config<'_>, color: Rgb) -> Result<Ws2812bIndicator<'_>, Error> {
    ...
    Ws2812b::new(RmtPixelWriter { channel }, color).await
}
...
impl PixelWriter for RmtPixelWriter<'_> {
    type Error = Error;
